...
| 2026-07-09/10 | **Go-Live ✅** | Neues, eigenes Supabase-Projekt `czmjxsojbomkqtluzhru` (EU; das alte `yacrrwyvrswssqurfhun` ist Lovable Cloud → verworfen). Baseline + Seed via `supabase db push --db-url … --include-all --include-seed` deployed; echter Account registriert; `migrate:v1` live (316 Weine, 268 Flaschen, CHF 9'574 — exakt wie Dry-Run). Frontend auf Vercel: **https://mywine-cellar.vercel.app** (auto-deploy bei Push auf `main`); GitHub-Remote bereinigt (v1-Historie → Branch `lovable-v1-archive`). **1 kritischer Bug gefunden & behoben (338d4b8):** `fetchMembership` ohne `.eq("user_id",…)` + `.maybeSingle()` → crash sobald Keller >1 Mitglied (RLS liefert alle Mitglieder). Beide reale Accounts verifiziert. |
| 2026-07-11 | **Geo-Hygiene-Fix ✅** | Vom User gemeldete Appellations-Fehler behoben (Detail: `.claude/plans/bevor-mit-phase-6-*.md`): Bogus `IGP Vin de Pays d'Oc` (Beaujolais) gelöscht; 16 Kürzel-Präfixe aus Namen entfernt (Typ steht im `type`-Feld, Picker zeigt Badge); Sub-Region-Kopien konsolidiert (Champagne 5×, Mosel/Baden 4×, Rioja/Douro …) → je 1 region-level Eintrag; `English Wine`/`South Eastern Australia` → country-level; `Vin de France` type=null; Schweizer Typen normalisiert (`AOC Valais`→`AOC`); Apostrophe/Akzente vereinheitlicht. 884→845 Appellationen. Picker zeigt country-level Appellationen jetzt auf jeder Ebene. Migration `20260711090000_geo_cleanup.sql` (idempotent, Weine-FKs werden umgehängt, redundante `classification`-Kürzel bereinigt); **Konvergenz verifiziert:** alter Seed + Migration ≡ neuer Seed (Diff leer), 5 simulierte Wein-Fälle korrekt. |
| 2026-07-13 | **Geo-Perfektion ✅ (Voll-Audit)** | User meldete weitere Picker-Fehler → Voll-Audit + Neubau (Detail: `.claude/plans/bevor-mit-phase-6-*.md`). **Picker-Root-Cause gefunden (live reproduziert):** Radix-Select feuert spontanes `onValueChange("")`, wenn ihm programmatisch ein Wert gesetzt wird, zu dem noch kein Item gerendert war → Erstklick auf region-level Appellationen verlor Region+Appellation wieder. **Neubau:** reine Auswahl-Logik in `src/features/geography/selection.ts` (`resolveSelection` = eine Funktion für alle 4 Felder: Vorfahren auffüllen, unverträgliche Nachfahren löschen, unmögliche Werte verwerfen; `appellationOptions` mit Mehrfach-Anker-Dedupe), GeographyPicker-Handler = Einzeiler, cmdk `value`=UUID + Teilstring-Suche über keywords, 19 Unit-Tests (`selection.test.ts`). Widersprüche (Pomerol+Medoc) unmöglich. **Daten:** 71 eponyme Einzel-Subs aufgelöst (ID-erhaltende Moves), FR-Struktur repariert (Bourgogne/Alsace/Cotes-du-Rhone-Dups, Haut-Medoc→Medoc, Crus du Beaujolais, Vire-Clesse), 17 „IGT "-/10 „Vino de la Tierra "-/9 „Vinho Regional "-Präfixe gestrippt, erfundene Einträge gelöscht („IGT Langhe Fantasia", 16 Pseudo-Tafelweine; Guard: nur wenn unreferenziert), falsche Eltern korrigiert (VT Castilla→C-LM, Transmontano→Tras-os-Montes), DE-Typ „Anbaugebiet", AT Wachau/Thermenregion/Ruster Ausbruch→DAC. 845→820. **Invarianten jetzt im geo:build-Validator** (Build bricht bei Präfix/Duplikat/eponymer Sub/’ ab). Migration `20260713090000_geo_perfection.sql`; Konvergenz alter Seed+Migration ≡ neuer Seed (Diff leer), 7 simulierte Wein-Fälle korrekt. |
| — | 6–9 | Nicht begonnen. Als Nächstes gemäß §9: **Phase 6** (Edge Function v2: Auth, Tool-Use-JSON, V7 Sommelier, V8 Trinkfenster). |

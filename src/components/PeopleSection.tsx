...
export const PeopleSection = () => {
  ...
  const renameMut = useRenamePerson();
  const deleteMut = useDeletePerson();

  const [newName, setNewName] = useState("");
  const [newAvatar, setNewAvatar] = useState(PERSON_EMOJI_OPTIONS[0]);

  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingName, setEditingName] = useState("");
  const [editingAvatar, setEditingAvatar] = useState(PERSON_EMOJI_OPTIONS[0]);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.error(e.message ?? "Could not add person");
    }
  };
